...
- **root `src/main.rs`** — CLI (`clap`), config loading/auto-generation, tracing init (with a `tracing_log::LogTracer` bridge so the `log` macros re-exported via the prelude reach the fmt subscriber), and the top-level `tokio::select!` shutdown loop (ctrl-c / SIGQUIT / SIGTERM / internal broadcast). Shutdown uses a `broadcast::Sender<String>` (disconnect reason) + `mpsc::Sender<String>` (`shutdown_complete_tx`) so the server waits for all connection tasks to drop before exiting. `src/quit.rs` wraps the OS signal futures.
